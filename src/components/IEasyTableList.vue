      ...
      <a-config-provider :locale="locale">
        ...
        >
          ...
          <template v-for="(item, cindex) in columns">
            ...
            <div v-if="propData.expandedRowRender&&propData.expandedRowRender.length>0" :slot="`expandedRowRender${propData.expandedRowRender&&propData.expandedRowRender.length>0?'':'close'}`" slot-scope="record,index,indent,expanded">
              <div v-if="propData.expandedRowRenderReturnType&&propData.expandedRowRenderReturnType=='text'">
                {{ getRowsExpandedCustomRender(record,index,indent,expanded) }}
              </div>
              <div v-else-if="propData.expandedRowRenderReturnType&&propData.expandedRowRenderReturnType=='html'"
              v-html="getRowsExpandedCustomRender(record,index,indent,expanded)">
              </div>
            </div>
...
export default {
  ...
  computed: {
        ...
        this.propData.columnsList.forEach((item) => {
          ...
          titleCustomRender &&
            titleCustomRender.forEach((fitem) => {
              ...
              columnObj.slots = {
                title: "title_" + (columnObj.key || columnObj.dataIndex||IDM.uuid()),
              };
            });
    ...
    rowSelection() {
      ...
      console.log("🚀 ~ file: IEasyTableList.vue ~ line 227 ~ rowSelection ~ this", this)
      ...
      //设置是否去掉『全选』『反选』两个默认选项
      rowSelectionConfig.hideDefaultSelections = this.propData.hideDefaultSelections||false
      //单选/多选
      ...
      //手动选择/取消某列的自定义函数
      rowSelectionConfig.onSelect = function(record, selected, selectedRows, nativeEvent) {
      ...
      rowSelectionConfig.onSelectInvert = function(selectedRows) {
        var rowSelectionOnSelectInvert = that.propData.rowSelectionOnSelectInvert;
  ...
  methods: {
     ...
     */
    getRowsExpandedCustomRender(record,index,indent,expanded){
      return window[this.propData.expandedRowRender[0].name] &&
    ...
    getRowsCustomRender(item,text,record,index){
      return window[item.rowsCustomRender.name] &&
    ...
    customRow(record, index) {
      ...
      return {
        ...
        on: {
          ...
          dblclick: (event) => {that.customRowFunHandle(record, index,event,"customRowDbClick")},
          contextmenu: (event) => {that.customRowFunHandle(record, index,event,"customRowContextMenu")},
          mouseenter: (event) => {that.customRowFunHandle(record, index,event,"customRowMouseenter")}, // 鼠标移入行
          mouseleave: (event) => {that.customRowFunHandle(record, index,event,"customRowMouseleave")},
        },
      };
    },
    /**
    ...
    rowClassNameHandle(record, index){
      let that = this;
      if(that.propData.rowClassNameFunction&&that.propData.rowClassNameFunction.length>0){
        return window[that.propData.rowClassNameFunction[0].name] &&
